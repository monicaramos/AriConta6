        ...
        Case 3 'INSERTAR
            If DatosOK Then
                ...
                If Mc.ConseguirContador(Trim(Text1(2).Text), (i = 0), False) = 0 Then
                    ...
                    If B Then
                        'LOG
                        vLog.Insertar 4, vUsu, Text1(2).Text & Text1(0).Text & " " & Text1(1).Text
...
Dim Rs As ADODB.Recordset
    ...
    SQL = "select numasien, fechaent from pagos where numserie = " & DBSet(Serie, "T")
    SQL = SQL & " and codmacta = " & DBSet(Cuenta, "T")
    SQL = SQL & " and numfactu = " & DBSet(FACTURA, "T")
    SQL = SQL & " and fecfactu = " & DBSet(FecFactu, "F")
...
Dim SQL As String
    ...
    CadInsert = "insert into pagos (numserie,codmacta,numfactu,fecfactu,numorden,codforpa,fecefect,impefect," & _
                "ctabanc1,entidad,oficina,control,cuentaba,fecultpa,imppagad,emitdocum,contdocu," & _
                "text1csb,text2csb,transfer,referencia, iban) values "
    CadValues = ""
...
eInsertarPagos:
    MuestraError Err.Number, "Insertar Pagos", Err.Description
End Function
...
Dim i As Integer
    ...
    For i = 0 To imgppal.Count - 1
        If i <> 0 And i <> 7 And i <> 9 Then imgppal(i).Picture = frmPpal.ImageList3.ListImages(1).Picture
    Next i
...
Dim B As Boolean
    ...
    imgppal(8).Visible = (Data1.Recordset.RecordCount > 1)
...
Private Sub frmPag_DatoSeleccionado(CadenaSeleccion As String)
    If CadenaSeleccion <> "" Then
        ...
        Entidad = RecuperaValor(CadenaSeleccion, 2)
        Oficina = RecuperaValor(CadenaSeleccion, 3)
        Control = RecuperaValor(CadenaSeleccion, 4)
        Cuentaba = RecuperaValor(CadenaSeleccion, 5)
...
Dim cWhere As String
    ...
    cWhere = "(numserie, numregis, fecfactu) in (select factpro.numserie, factpro.numregis, factpro.fecharec from "
    cWhere = cWhere & "factpro LEFT JOIN factpro_lineas ON factpro.numserie = factpro_lineas.numserie and factpro.fecharec = factpro_lineas.fecharec and factpro.numregis = factpro_lineas.numregis "
...
'    If Not SePuedeModificarAsiento(True) Then Exit Sub
    ...
    If Not IsNull(Data1.Recordset!NumAsien) Then
        ...
        If Numasien2 = 0 Then
            ...
            Exit Sub
        End If
' Ya no preguntamos si quiere desactualizarla
'        SQL = "Esta factura ya está contabilizada. Desea desactualizar para poder modificarla?"
...
Private Sub BotonEliminar(EliminarDesdeActualizar As Boolean)
    ...
    Else
        ...
        If BorrarFactura Then
            'LOG
            vLog.Insertar 6, vUsu, CStr(DBLet(Data1.Recordset!NumSerie)) & Format(i, "000000")
...
'    PosicionarCombo Combo1(0), Text1(22).Text
...
'    cmdSaldoHco(0).Visible = Not AdoAux(1).Recordset.EOF
'    cmdSaldoHco(1).Visible = Not AdoAux(1).Recordset.EOF
...
'            'letra distinta
'            'ASignaremos contador, si la fecha esta puesta
'            If Text1(1).Text <> "" Then
'                Correcto = CDate(Text1(1).Text) <= vParam.fechafin
'                If Text1(0).Text <> "" Then
'                    L = Val(Text1(0).Text)
'                Else
'                    L = 0
'                End If
'                If Mc.ConseguirContador(Text1(Index).Text, Correcto, False) = 0 Then
'                    Text1(0).Text = Mc.Contador
'                Else
'                    MsgBox "La letra no es de contadores: " & Text1(Index).Text, vbExclamation
'                    Text1(Index).Text = ""
'                    Text1(0).Text = ""
'                    PonFoco Text1(2)
'                End If
...
' No preguntamos si quiere desactualizar
'        SQL = "Esta factura ya está contabilizada. Desea desactualizar para poder modificarla?"
'        If MsgBox(SQL, vbQuestion + vbYesNoCancel) <> vbYes Then Exit Sub
        Numasien2 = Data1.Recordset!NumAsien
        NumDiario = Data1.Recordset!NumDiari
...
'            End If
'            txtAux2(28).Text = DBLet(AdoAux(Index).Recordset!deformapago, "T")
...
'        ToolbarAux.Buttons(5).Enabled = DBLet(Rs!Ver, "N") And (Modo = 2 And Me.Data1.Recordset.RecordCount > 0) And DesdeNorma43 = 0
'        ToolbarAux.Buttons(6).Enabled = DBLet(Rs!Ver, "N") And (Modo = 2 And Me.Data1.Recordset.RecordCount > 0) And DesdeNorma43 = 0
'        ToolbarAux.Buttons(7).Enabled = DBLet(Rs!creareliminar, "N") And (Modo = 2 And Me.Data1.Recordset.RecordCount > 0) And DesdeNorma43 = 0
...
'            If txtAux(Index).Text = "" Then Exit Sub
            ...
            RC = "idsubcos"
            SQL = DevuelveDesdeBD("nomccost", "ccoste", "codccost", txtaux(12).Text, "T", RC)
            txtAux2(12).Text = ""
            If SQL = "" Then
...
Dim Sql4 As String
    ...
    While Not Rs.EOF
        ...
        Sql4 = "update pagos set numdiari = " & DBSet(FP.diaricli, "N")
        Sql4 = Sql4 & ", fechaent =  " & DBSet(FechaPago, "F")
        Sql4 = Sql4 & ", numasien = " & DBSet(Mc.Contador, "N")
        Sql4 = Sql4 & ", usuariopago = " & DBSet(vUsu.Nombre, "T")
        Sql4 = Sql4 & " where numserie = " & DBSet(Rs!NumSerie, "T")
        Sql4 = Sql4 & " and codmacta = " & DBSet(Rs!codmacta, "T")
        Sql4 = Sql4 & " and numfactu = " & DBSet(Rs!NumFactu, "T")
        Sql4 = Sql4 & " and fecfactu = " & DBSet(Rs!FecFactu, "F")
        Sql4 = Sql4 & " and numorden = " & DBSet(Rs!numorden, "N")
